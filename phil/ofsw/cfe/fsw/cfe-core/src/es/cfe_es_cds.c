...
/* except for the mempool which is guaranteed to start at an 8-byte offset boundary, */
...
#define CDS_POOL_OFFSET     (((CDS_REG_OFFSET + (CFE_ES_CDS_MAX_NUM_ENTRIES * sizeof(CFE_ES_CDS_RegRec_t))) + 7) & 0xfffffff8)
...
int32 CFE_ES_RebuildCDS(void)
{
        ...
        (CFE_ES_Global.CDSVars.MaxNumRegEntries <= CFE_ES_CDS_MAX_NUM_ENTRIES))
    {
        ...
        if (Status == OS_SUCCESS)
        {
            ...
            PoolOffset = (CDS_REG_OFFSET + (CFE_ES_Global.CDSVars.MaxNumRegEntries * sizeof(CFE_ES_CDS_RegRec_t)) + 7) & 0xfffffff8;;
