...
mc_prefix_s = 'mtv_slosh_mc';
mc_prefix_s_pos = 'mtv_slosh_pos_mc';
mc_prefix_workspace = 'mtv_slosh_workspace_mc';
...
pendulum_mass_min   = 0.8;
pendulum_mass_max   = 1.2;
pendulum_mass       = pendulum_mass_min + (pendulum_mass_max - pendulum_mass_min)*rand(mc_n,1);

% set peak hinge height/tank diameter ratio, nominally set to 100%
hinge_height_min = 0.8;
hinge_height_max = 1.2;
...
%     imh_gyro_bias_noise_on  = 1;  % 1 - ON, 0 - OFF
    ...
    % %%%%%%%%%%%%%
        ...
        if ~isnan( mc_6dof_variables(imc,ivars.pendulum_mass) ) && ( mc_6dof_variables(imc,ivars.pendulum_mass) ~= 1.0 )
            [mm,ii]=max(SloshData.pendulum_mass);
            SloshData.pendulum_mass     = mc_6dof_variables(imc,ivars.pendulum_mass) * interp1([0 SloshData.fill_fraction(ii) 1],[0 mm 0],SloshData.fill_fraction);
            SloshData.pendulum_mass(1)  = 1e-6;
            SloshData.pendulum_mass(end)= 1e-6;
        end
