...
void CFE_TIME_SetTimeCmd(CFE_SB_MsgPtr_t MessagePtr)
{
    ...
    */
    if (CommandPtr->MicroSeconds < 1000000)
    {
        ...
        OldTime = CFE_TIME_GetTime();
        CFE_TIME_SetTime(NewTime);

        CFE_TIME_TaskData.CmdCounter++;
        if (CFE_TIME_Compare(NewTime, OldTime) == CFE_TIME_A_LT_B) {
            Sign = '-';
            DeltaTime = CFE_TIME_Subtract(OldTime, NewTime);
        } else {
            Sign = '+';
            DeltaTime = CFE_TIME_Subtract(NewTime, OldTime);
        }

        CFE_EVS_SendEvent(CFE_TIME_TIME_EID, CFE_EVS_INFORMATION,
                         "Set Time -- secs = %d, usecs = %d, ssecs = 0x%X (delta = %c%d.%06ds)",
                          CommandPtr->Seconds, CommandPtr->MicroSeconds,
                          CFE_TIME_Micro2SubSecs(CommandPtr->MicroSeconds),
                          Sign, DeltaTime.Seconds,
                          CFE_TIME_Sub2MicroSecs(DeltaTime.Subseconds));
