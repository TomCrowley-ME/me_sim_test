...
function [ contour_z, n_runs ] = create_contour_matrix(ifi_name_string, axis_name, data_struct_a, data_struct_b)
...
for ii = 1 : n_bins
    for jj = 1 : n_bins
        ...
        if ~isempty(C)
            eval(['contour_z(ii,jj) = mean(normrows(mc_all_final(C,ifi.' ifi_name_string '(' num_string '))),1);'])
