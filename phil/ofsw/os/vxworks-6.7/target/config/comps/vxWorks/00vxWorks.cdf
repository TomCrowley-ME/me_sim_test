...
/*
 * Copyright (c) 1998-2008 Wind River Systems, Inc.
 *
...
11k,08dec06,jpb  Fixed warning being issued that parameter has bad value.  Type
                 needs to be 'BOOL' and not 'bool' (Defect #37172).
...
10d,19jan06,xli  add the INCLUDE_SOCKLIB to usrRoot init group since INCLUDE_SOCKLIB
...
Folder	FOLDER_ANSI {
	...
	DEFAULTS	INCLUDE_ANSI_ASSERT INCLUDE_ANSI_CTYPE  \
			...
			INCLUDE_ANSI_TIME INCLUDE_ANSI_STRDUP   \
			INCLUDE_ANSI_STRERROR
}
...
Component INCLUDE_ANSI_CTYPE {
	...
	LINK_SYMS	__ctype isalnum isalpha iscntrl isdigit isgraph islower \
...
Component INCLUDE_POSIX_CLOCKS {
	...
	MODULES		clockLib.o
	PROTOTYPE       extern void clockLibInit (void);
...
Folder	FOLDER_APPLICATION {
	...
	CHILDREN	INCLUDE_USER_APPL \
                        ...
                        INCLUDE_USER_POST_NETWORK_APPL_INIT
...
Component INCLUDE_USER_POST_NETWORK_APPL_INIT {
        NAME            post-network application initialization
        SYNOPSIS        call usrPostNetworkAppInit() (in your usrAppInit.c project file)
        INIT_RTN        usrPostNetworkAppInit ();
}
...
Folder	FOLDER_KERNEL {
	...
	CHILDREN	INCLUDE_KERNEL		\
			...
			INCLUDE_SEM_MINIMAL_BINARY	 	\
			INCLUDE_SEM_MINIMAL_MUTEX	 	\
			INCLUDE_SEM_BINARY_CREATE		\
			INCLUDE_SEM_MUTEX_CREATE		\
			INCLUDE_SEM_MINIMAL_BINARY_CREATE	\
			INCLUDE_SEM_MINIMAL_MUTEX_CREATE	\
			INCLUDE_SEM_COUNTING_CREATE		\
			...
			INCLUDE_EXC_TASK	\
			INCLUDE_TLS
	DEFAULTS	INCLUDE_KERNEL INCLUDE_EXC_HANDLING \
			INCLUDE_MEM_MGR_FULL INCLUDE_MEM_MGR_INFO \
			INCLUDE_OBJ_LIB \
			INCLUDE_SEM_BINARY INCLUDE_SEM_MUTEX \
			INCLUDE_SEM_COUNTING INCLUDE_SEM_BINARY_CREATE \
			INCLUDE_SEM_COUNTING_CREATE INCLUDE_SEM_MUTEX_CREATE \
			INCLUDE_SEM_DELETE INCLUDE_SIGNALS \
			INCLUDE_MSG_Q INCLUDE_MSG_Q_CREATE_DELETE \
			INCLUDE_TASK_HOOKS INCLUDE_TASK_UTIL \
			INCLUDE_WATCHDOGS INCLUDE_WATCHDOGS_CREATE_DELETE \
			INCLUDE_ENV_VARS INCLUDE_EXC_TASK INCLUDE_JOB_TASK \
			INCLUDE_TASK_CREATE_DELETE INCLUDE_TASK_RESTART \
			INCLUDE_HOOKS INCLUDE_REBOOT_HOOKS \
			SELECT_KERNEL_SCHEDULER INCLUDE_VXMEMPROBE_INIT
}
...
Component INCLUDE_KERNEL {
	...
	HDR_FILES	sysLib.h private/kernelLibP.h private/workQLibP.h \
			qPriBMapLib.h taskLib.h usrConfig.h private/isrLibP.h \
			private/excLibP.h
}
...
Component INCLUDE_ENV_VARS {
	NAME		unix compatable environment variables
	SOURCE		$(TGT_DIR)/src/os
	MODULES		envLib.o
...
Component INCLUDE_JOB_TASK {
	...
	SOURCE		$(TGT_DIR)/src/os
        MODULES         jobLib.o
        INIT_RTN        jobLibInit (JOB_TASK_STACK_SIZE);
...
Parameter FUNCPTR_OBJ_MEMALLOC_RTN {
        ...
        TYPE            void *
        DEFAULT         (INCLUDE_MEM_ALLOT)::(memAllot) memPartAlloc
}
...
Component INCLUDE_OBJ_LIB {
	...
	REQUIRES	INCLUDE_ANSI_STRNCPY \
			...
			INCLUDE_KERNEL       \
			INCLUDE_SEM_MUTEX
	INIT_RTN	objLibInit ((FUNCPTR)FUNCPTR_OBJ_MEMALLOC_RTN,	\
...
Component INCLUDE_SEM_MINIMAL_BINARY {
	NAME		minimal binary semaphores
	SOURCE		$(TGT_DIR)/src/wind
	LINK_SYMS	semBGive
	HDR_FILES	private/semMinimalLibP.h
	REQUIRES	INCLUDE_SEM_LIB
}

Component INCLUDE_SEM_MINIMAL_MUTEX {
	NAME		minimal mutex semaphores
	SOURCE		$(TGT_DIR)/src/wind
	LINK_SYMS	semMGive
	HDR_FILES	private/semMinimalLibP.h
	REQUIRES	INCLUDE_SEM_LIB
...
Component INCLUDE_SEM_MINIMAL_BINARY_CREATE {
	NAME		optimized binary semaphore creation routine
	SOURCE		$(TGT_DIR)/src/wind
	LINK_SYMS	semBCreate
	HDR_FILES	private/semMinimalLibP.h
	REQUIRES	INCLUDE_SEM_MINIMAL_BINARY \
			INCLUDE_OBJ_LIB
}
...
Component INCLUDE_SEM_MINIMAL_MUTEX_CREATE {
	NAME		optimized mutex semaphore creation routine
	SOURCE		$(TGT_DIR)/src/wind
	LINK_SYMS	semMCreate
	HDR_FILES	private/semMinimalLibP.h
	REQUIRES	INCLUDE_SEM_MINIMAL_MUTEX \
			INCLUDE_OBJ_LIB
}
...
Component INCLUDE_EDR_SYSDBG_FLAG {
	...
	INIT_RTN	edrSystemDebugModeInit ();
	HDR_FILES	edrLib.h
}
...
Component INCLUDE_PX_SCHED_DEF_POLICIES {
        ...
        SYNOPSIS        Provide support for POSIX SCHED_FIFO, SCHED_RR, \
			  and SCHED_OTHER policies in RTPs. Selecting this component will \
...
Component INCLUDE_PX_SCHED_SPORADIC_POLICY {
        ...
        SYNOPSIS        Provide support for POSIX SCHED_FIFO, SCHED_RR, \
			  SCHED_OTHER, and SCHED_SPORADIC policies in RTPs. Selecting this component \
...
Folder	FOLDER_IO_SYSTEM {
	...
	CHILDREN	INCLUDE_IO_SYSTEM	\
			...
			INCLUDE_VRFS		\
                        INCLUDE_UTF             \
			INCLUDE_TTY_DEV
	DEFAULTS	INCLUDE_IO_SYSTEM INCLUDE_STDIO INCLUDE_FLOATING_POINT \
			INCLUDE_FORMATTED_IO INCLUDE_FORMATTED_OUT_BASIC \
			INCLUDE_LOGGING INCLUDE_PIPES \
			INCLUDE_SELECT INCLUDE_TTY_DEV
}
...
Parameter IOS_POSIX_PSE52_MODE {
        ...
        SYNOPSIS        Enables POSIX PSE52 behaviour for the VxWorks I/O subsystem
        TYPE            BOOL
        DEFAULT         (INCLUDE_RTP_POSIX_PSE52)::(TRUE) FALSE
}
...
Component INCLUDE_NBIO_LOG {
	NAME		non-blocking message logging
	SOURCE		$(TGT_DIR)/src/os
	MODULES		nbioLogLib.o                \
			nbioLogLibApi.o
	INIT_RTN	nbioLogInit (consoleFd, INIT_NBIO_MSGS);
	CFG_PARAMS	INIT_NBIO_MSGS
	HDR_FILES	private/nbioLogLibP.h
...
Component INCLUDE_STRING_SEARCH {
	...
	MODULES		strSearchLib.o
	LINK_SYMS	fastStrSearch
}
...
Component INCLUDE_EVENTPOINT_STUB {
	...
	HDR_FILES	private/eventPointStubLibP.h
}
...
Component INCLUDE_POSIX_TRACE {
	...
	_CHILDREN	FOLDER_POSIX
	REQUIRES	INCLUDE_RTP \
                        INCLUDE_POSIX_CLOCKS INCLUDE_POSIX_TRACE_TIMESTAMP
}
...
Component INCLUDE_RBUFF {
	...
	INCLUDE_WHEN	INCLUDE_WINDVIEW
        REQUIRES        INCLUDE_SPINLOCK_ISR_ND
}
...
Component INCLUDE_Q_LIB {
	NAME		Generic queue type infrastructure
	SOURCE		$(TGT_DIR)/src/util
	MODULES		qLib.o
	REQUIRES	INCLUDE_KERNEL
	_CHILDREN	FOLDER_NOT_VISIBLE
...
Component INCLUDE_Q_FIFO_CREATE_DELETE {
	NAME		Fifo queue dynamic creation and deletion
	SOURCE		$(TGT_DIR)/src/util
	MODULES		qFifoCreateLib.o
	INIT_RTN	qFifoCreateLibInit ();
	REQUIRES	INCLUDE_MEM_MGR_BASIC \
			INCLUDE_Q_FIFO
	_CHILDREN	FOLDER_NOT_VISIBLE
	PROTOTYPE	void qFifoCreateLibInit (void);
}

Component INCLUDE_Q_PRI_LIST_CREATE_DELETE {
	NAME		Priority ordered linked list queue library
	SOURCE		$(TGT_DIR)/src/util
	MODULES		qPriListCreateLib.o
	INIT_RTN	qPriListCreateLibInit ();
	REQUIRES	INCLUDE_MEM_MGR_BASIC \
			INCLUDE_Q_PRI_LIST
	_CHILDREN	FOLDER_NOT_VISIBLE
	PROTOTYPE	void qPriListCreateLibInit (void);
}

Component INCLUDE_Q_PRI_BMAP_CREATE_DELETE {
	NAME            Bit mapped priority queue management library
	SOURCE		$(TGT_DIR)/src/util
	MODULES		qPriBMapCreateLib.o
	INIT_RTN	qPriBMapCreateLibInit ();
	REQUIRES	INCLUDE_MEM_MGR_BASIC \
			INCLUDE_Q_PRI_BMAP
	_CHILDREN	FOLDER_NOT_VISIBLE
	PROTOTYPE	void qPriBMapCreateLibInit (void);
}

Component INCLUDE_Q_PRI_DELTA_CREATE_DELETE {
	NAME
	SOURCE		$(TGT_DIR)/src/util
	MODULES		qPriDeltaCreateLib.o
	INIT_RTN	qPriDeltaCreateLibInit ();
	REQUIRES	INCLUDE_MEM_MGR_BASIC \
			INCLUDE_Q_PRI_DELTA
	_CHILDREN	FOLDER_NOT_VISIBLE
	PROTOTYPE	void qPriDeltaCreateLibInit (void);
}

Component INCLUDE_Q_CREATE_DELETE {
	NAME
	SOURCE		$(TGT_DIR)/src/util
	MODULES		qCreateLib.o
	REQUIRES	INCLUDE_MEM_MGR_BASIC \
			INCLUDE_Q_LIB
	_CHILDREN	FOLDER_NOT_VISIBLE
	LINK_SYMS	qCreate qDelete
}
...
Component INCLUDE_SET {
        NAME            logical set library
        SYNOPSIS        sets of unspecified item types
	SOURCE		$(TGT_DIR)/src/util
        MODULES         setLib.o
        INIT_RTN        setLibInit ();
        HDR_FILES       setLib.h
...
InitGroup usrKernelCoreInit {
	...
	INIT_ORDER	INCLUDE_VXEVENTS			\
			...
			INCLUDE_MSG_Q_CREATE_DELETE 		\
			INCLUDE_WATCHDOGS_CREATE_DELETE 	\
			INCLUDE_Q_FIFO_CREATE_DELETE 		\
			INCLUDE_Q_PRI_BMAP_CREATE_DELETE	\
			INCLUDE_Q_PRI_DELTA_CREATE_DELETE	\
			INCLUDE_Q_PRI_LIST_CREATE_DELETE 	\
...
Parameter TASK_PRIORITY_SET_COMPATIBLE {
	...
	SYNOPSIS	This configuration parameter can be used to enable \
			...
			a running task may be preemted when it's priority \
...
Parameter INIT_NBIO_MSGS {
	NAME		initial # NBIO buffers
	SYNOPSIS	initial allocation of 128-byte NBIO buffers
	TYPE		uint
	DEFAULT		32
}
...
Parameter ENABLE_ALL_CPUS {
	NAME		Enable all CPUs configured CPUs for SMP
	TYPE		BOOL
	DEFAULT		TRUE
	_CFG_PARAMS	INCLUDE_KERNEL
}
