...
% nimbus_gen_code.m                                                       %
%                                                                         %
% Generates code for the nimbus model. Each argument is interpreted as a  %
...
%      -r "fsw_gen_code('ofsw', 'headers')"                               %
%                                                                         %
% 10/31/13                                                                %
% mikestewart@moonexpress.com                                             %
%-------------------------------------------------------------------------%

function nimbus_gen_code(varargin)
...
if generate_all_ofsw || generate_all_gsse || ~isempty(job_list)
    ...
    % subsystems
    blks = find_system('nimbus','SearchDepth','2','FollowLinks','on','BlockType','SubSystem');
...
if generate_headers
    ...
    if isempty(successful_targets)
        ...
        try
            set_param('nimbus','SimulationCommand','update');
            model_built = true;
        catch err
    ...
    if model_built
        fsw_get_model_rates('inc/csc_model_hz.h');
