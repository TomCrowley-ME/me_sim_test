...
function csu_cmc_convert_measured_currents_collection_generate
...
sys = 'csu_cmc_convert_measured_currents_collection_lib';
subsys = [sys '/csu_cmc_convert_measured_currents_collection'];


try
    load_system(sys);
catch
    error('Cannot load csu_cmc_convert_measured_currents_collection_lib. Cannot proceed.')
end

% At this point, we have the existing library open -- nuke the subsystem so we can start fresh
fprintf(1, 'Regenerating csu_cmc_convert_measured_currents_collection_lib...\n');
...
[bus_signal_names, conversion_map] = create_analog_conversion_maps('curr');
...
add_block('built-in/BusCreator', [subsys '/curr_bus'], 'Inputs', int2str(n_signals),'ShowName', 'off', 'Position', [1100 128 1105 (128+temp_concat_h)]);
add_block('built-in/Outport', [subsys '/curr_bus_out'], 'BackgroundColor', 'red', 'Port', '1', 'Position', [1150 (128+temp_concat_h/2-7) 1180 (128+temp_concat_h/2+7)]);
add_line(subsys, 'curr_bus/1', 'curr_bus_out/1');
...
for i=1:n_signals
    ...
    add_block('simulink/Signal Attributes/Data Type Conversion', [subsys '/cmc_double_' i_str], 'Position', csu_tdl_pos - [100 0 100 0]);
    set_param([subsys '/cmc_double_' i_str], 'OutDataTypeStr', 'double');
    ...
    add_block('simulink/User-Defined Functions/Fcn', [subsys '/cmc_fcn_' i_str], 'Expr', fcn_expr, 'Position', csu_tdl_pos);
    
    add_line(subsys, ['count_bus/' i_str], ['cmc_double_' i_str '/1']);
    
    add_line(subsys, ['cmc_double_' i_str '/1'], ['cmc_fcn_' i_str '/1']);
    
    add_line(subsys, ['cmc_fcn_' i_str '/1'], ['curr_bus/' i_str]);
    
    port_handles = get_param([subsys '/cmc_fcn_' i_str], 'PortHandles');
