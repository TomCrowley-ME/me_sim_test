...
%
%       addmulti
%           Adds multiple pictures to the current slide. Requires a
%           page-length structure array of image filenames be supplied.
%           Example: input(# of pages).filenames(# of images per page)
%           This command does not return any values.
%
%       addtext
...
%               FontColor   Three element vector specifying RGB value in the
...
%               Shadow      Controls adding shadow behind text:
%                           off - no shadow (default)
%                           on
%               Outline     Controls adding outline around text:
%                           off - no outline (default)
%                           on
%               OutlineColor     Three element vector specifying RGB value
%                           in the range from 0 to 1. Default outline color
%                           is white.
...
switch lower(action),
    ...
    case 'addmulti',
        ...
        %% Inputs
        if nargin<2,
            error('exportToPPTX:minInput','Second argument required: array of image file names');
        end
        imgData     = varargin{2};
        numFigs     = size(imgData,1);

        %% Define positions and sizes corresponding to number of images to be added to current slide
        figs(1).Pos=cat(1,[3.00 1.00 5.10 4.4]);
        figs(2).Pos=cat(1,[0.66 1.00 4.67 4.4], ...
                          [5.33 1.00 4.67 4.4]);
        figs(3).Pos=cat(1,[0.39 1.63 3.20 3.0], ...
                          [3.59 1.63 3.20 3.0], ...
                          [6.79 1.63 3.20 3.0]);
        figs(4).Pos=cat(1,[1.58 1.00 4.00 2.2], ...
                          [5.58 1.00 4.00 2.2], ...
                          [1.58 3.20 4.00 2.2], ...
                          [5.58 3.20 4.00 2.2]);
        figs(5).Pos=cat(1,[1.21 1.00 2.93 2.2], ...
                          [4.14 1.00 2.93 2.2], ...
                          [1.21 3.20 2.93 2.2], ...
                          [4.14 3.20 2.93 2.2], ...
                          [7.07 2.07 2.93 2.2]);
        figs(6).Pos=cat(1,[1.21 1.00 2.93 2.2], ...
                          [4.14 1.00 2.93 2.2], ...
                          [7.07 1.00 2.93 2.2], ...
                          [1.21 3.20 2.93 2.2], ...
                          [4.14 3.20 2.93 2.2], ...
        ...
        for ii=1:numFigs
            % image files
            if ischar(imgData(ii,:))
                file=deblank(imgData(ii,:));
                if exist(file)
                    exportToPPTX('addpicture',file,'Position',figs(numFigs).Pos(ii,:));
                else
                    fprintf('Image file not found: %s\n',file)
                end
            % figure handles    
            elseif ishghandle(imgData(ii),'Figure')
                exportToPPTX('addpicture',imgData(ii),'Position',figs(numFigs).Pos(ii,:));
            end
        end
...
fCol        = validateColor(getPVPair(varargin,'FontColor',[]));
...
switch lower(getPVPair(varargin,'Shadow','')),
    case 'on'
        sAlign  = {'algn','tl'};
    case {'off',''},
        sAlign  = {};
    otherwise,
        error('exportToPPTX:badProperty','Bad property value found in Shadow');
end

switch lower(getPVPair(varargin,'Outline','')),
    case 'on'
        oDash  = {'val','solid'};
    case {'off',''},
        oDash  = {};
    otherwise,
        error('exportToPPTX:badProperty','Bad property value found in Outline');
end
oCol = validateColor(getPVPair(varargin,'OutlineColor',[]));
...
for ipara=1:numParas,
    ...
    if ~isempty(paraText{ipara}),
        ...
        for irun=1:numRuns,
            ...
            if ~isempty(fCol)&&isempty(oDash)
                sf      = addNode(fileXML,rPr,'a:solidFill');
                addNode(fileXML,sf,'a:srgbClr',{'val',fCol});
            end
            if ~isempty(oDash)
                lnPtr    = addNode(fileXML,rPr,   'a:ln',{'w','6350','cmpd','sng'});
                sfPtr    = addNode(fileXML,lnPtr, 'a:solidFill');
                if ~isempty(oCol),
                    addNode(fileXML,sfPtr,'a:srgbClr',{'val',oCol});
                else
                    addNode(fileXML,sfPtr,'a:srgbClr',{'val','FFFFFF'});
                end
                          addNode(fileXML,lnPtr,  'a:prstDash',{oDash{:}});
                          addNode(fileXML,lnPtr,  'a:miter'   ,{'lim','800000'});
                sfPtr   = addNode(fileXML,rPr,    'a:solidFill');
                          addNode(fileXML,sfPtr,  'a:schemeClr',{'val','tx1'});
            end
            if ~isempty(sAlign)
                effPtr  = addNode(fileXML,rPr,    'a:effectLst');
                shdwPtr = addNode(fileXML,effPtr, 'a:outerShdw',{sAlign{:},'dir','2700000','dist','50800','blurRad','50800'});
                clrPtr  = addNode(fileXML,shdwPtr,'a:srgbClr'  ,{'val','000000'});
                          addNode(fileXML,clrPtr, 'a:alpha'    ,{'val','43137'});
            end
...
if length(size(imgData))>2 && numel(imgData)>1,
    % Image CDATA
...
% Set object ID
if isfield(PPTXInfo.Slide(PPTXInfo.numSlides),'objId')
    PPTXInfo.Slide(PPTXInfo.numSlides).objId    = PPTXInfo.Slide(PPTXInfo.numSlides).objId+1;
else
    fileName = PPTXInfo.Slide.file;
