...
/*      Make sure we're not loading any kernels that have already been loaded */
    for (i__ = 0; i__ < 1300; i__++) {
	if (s_cmp(files + (i__ < 1300 && 0 <= i__ ? i__ : 
		s_rnge("files", i__, "keeper_", (ftnlen)2587)) * 255, file, (
