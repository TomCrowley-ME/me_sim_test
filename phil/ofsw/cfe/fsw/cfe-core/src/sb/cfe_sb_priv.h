...
typedef struct {
     ...
     /* If ES mempool is aligned, make an effort to preserve that alignment */
#if defined(CFE_ES_MEMPOOL_ALIGNMENT) && (OS_MAX_API_NAME % CFE_ES_MEMPOOL_ALIGNMENT) != 0
     uint8             Spare[OS_MAX_API_NAME % CFE_ES_MEMPOOL_ALIGNMENT];
#endif
} CFE_SB_BufferD_t;
...
typedef struct {
   ...
   uint8 OS_ALIGN(8)  Partition[CFE_SB_BUF_MEMORY_BYTES];
