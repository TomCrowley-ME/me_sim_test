...
STATUS sdMmcCmdSendRelativeAddr
    ...
    if (card->isMmc)
        {
        SDMMC_CMD_FORM (cmd, SD_CMD_SEND_RELATIVE_ADDR, 0x1234 << 16, SDMMC_CMD_RSP_R1, FALSE);

        rc = sdMmcHostCmdIssue (card->host);
        if (rc == ERROR)
            return ERROR;

        card->info.rca = (UINT16)0x1234;
        }
    else
        {
...
/*******************************************************************************
*
* sdMmcCmdSwitch - issue CMD6 to modify EXT CSD of a card
*
* This routine issues CMD6 to modify EXT CSD of a card
*
* RETURNS: OK or ERROR
*
* ERRNO: N/A
...
STATUS sdMmcCmdSwitch
    ...
    arg = MMC_EXT_CSD_ARG_FORM(access, index, value, set);
    SDMMC_CMD_FORM (cmd, MMC_CMD_SWITCH, arg, SDMMC_CMD_RSP_R1B, FALSE);
...
/*******************************************************************************
*
* sdMmcCmdSendExtCsd - issue CMD8 to ask card to send its extended CSD
*
* This routine issues CMD8 to ask card to send its extended CSD
*
* RETURNS: OK or ERROR
*
* ERRNO: N/A
...
STATUS sdMmcCmdSendExtCsd
    ...
    buf = cacheDmaMalloc(512);

    SDMMC_CMD_FORM (cmd, MMC_CMD_SEND_EXT_CSD, 0, SDMMC_CMD_RSP_R1, TRUE);
    SDMMC_DATA_FORM (cmd, &buf[0], 1, SDMMC_BLOCK_SIZE, TRUE);

    rc = sdMmcHostCmdIssue (card->host);

    sector_count = (buf[215]<<24) | (buf[214]<<16) | (buf[213]<<8) | (buf[212]);
    if (sector_count != 0) {
        card->info.blkNum = sector_count;
        card->info.capacity = sector_count*512;
        card->info.highCapacity = TRUE;
    }
...
STATUS sdMmcCmdSendCsd
    ...
    if (card->isMmc)
        {
        mmcSpec = (card->info.csd[0] & 0x1c) >> 2;
        if (mmcSpec >= 4)
            {
...
/*******************************************************************************
*
* sdMmcSendOpCond - issue CMD1 to ask card to send its OCR
*
* This routine issues CMD1 to ask card to send its OCR.
*
* RETURNS: OK or ERROR
*
* ERRNO: N/A
...
STATUS sdMmcSendOpCond
    ...
    */
    ...
    SDMMC_CMD_FORM (cmd, MMC_CMD_SEND_OP_COND, arg, SDMMC_CMD_RSP_R3, FALSE);
...
STATUS sdMmcIdentify
    ...
    if (card->isMmc)
        ...
        while (1)
            ...
            if ((end - start) > (UINT64)(sysClkRateGet() * 2))
                {
                SDMMC_LIB_DBG (SDMMC_LIB_DBG_ERR, "CMD1 timeout\n",
                                0, 0, 0, 0, 0, 0);
                goto err;
                }
            }
    ...
    if (card->isMmc)
        {
        rc = sdMmcCmdSendExtCsd (card);
        if (rc == ERROR)
            goto err;

        rc = sdMmcCmdSwitch (card, EXT_CSD_WRITE_BYTE, EXT_CSD_BUS_WIDTH, SDMMC_BUS_WIDTH_4BIT, 0x0);
        if (rc == ERROR)
            goto err;

        sdMmcHostBusWidthSetup (card->host, SDMMC_BUS_WIDTH_4BIT);

        if (card->info.highSpeed)
            {
            rc = sdMmcCmdSwitch (card, EXT_CSD_WRITE_BYTE, EXT_CSD_HS_TIMING, 0x1, 0x0);
            if (rc == ERROR)
                goto err;

            sdMmcHostClkFreqSetup (card->host, SDMMC_CLK_FREQ_50MHZ);
            }
        else
            {
            sdMmcHostClkFreqSetup (card->host, SDMMC_CLK_FREQ_25MHZ);
            }
        }
    else
        {
