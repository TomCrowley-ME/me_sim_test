...
int32 OS_API_Init(void)
{
   ...
   for(i = 0; i < OS_MAX_TASKS; i++)
   {
        ...
        OS_task_table[i].events              = 0;
        OS_task_table[i].desired_events      = 0;
        OS_task_table[i].event_options       = 0;
    ...
    for(i = 0; i < OS_MAX_QUEUES; i++)
    {
        ...
        OS_queue_table[i].event_taskid = OS_MAX_TASKS;
        OS_queue_table[i].events       = 0;
    }
    ...
    for(i = 0; i < OS_MAX_BIN_SEMAPHORES; i++)
    {
        ...
        OS_bin_sem_table[i].event_taskid = OS_MAX_TASKS;
        OS_bin_sem_table[i].events       = 0;
    }
    ...
    for(i = 0; i < OS_MAX_COUNT_SEMAPHORES; i++)
    {
        ...
        OS_count_sem_table[i].event_taskid = OS_MAX_TASKS;
        OS_count_sem_table[i].events       = 0;
    }
                      ...
                      uint32 flags)
{
    ...
    */
    ...
    if (return_code != 0)
    {
       ...
       OS_InterruptSafeUnlock(&OS_task_table_mut, &previous);
       #ifdef OS_DEBUG_PRINTF
          printf("pthread_mutexattr_init error in OS_TaskCreate, Task ID = %d\n",possible_taskid);
       #endif
       return(OS_ERROR);
    }
    ...
    */
    ...
    if (return_code != 0)
    {
       ...
       OS_InterruptSafeUnlock(&OS_task_table_mut, &previous);
       #ifdef OS_DEBUG_PRINTF
          printf("pthread_mutexattr_setprotocol error in OS_TaskCreate, Task ID = %d\n",possible_taskid);
       #endif
       return(OS_ERROR);
    }
    ...
    */
    ...
    return_code = pthread_mutex_init(&(OS_task_table[possible_taskid].event_mutex), &mutex_attr);
    if(return_code != 0)
    {
       OS_task_table[possible_taskid].free = TRUE;
       OS_InterruptSafeUnlock(&OS_task_table_mut, &previous);
       #ifdef OS_DEBUG_PRINTF
          printf("pthread_mutex_create error in OS_TaskCreate, Task ID = %d\n",possible_taskid);
       #endif
       return(OS_ERROR);
    }

    return_code = pthread_cond_init(&(OS_task_table[possible_taskid].event_cond), NULL);
    if (return_code != 0)
    {
       OS_task_table[possible_taskid].free = TRUE;
       OS_InterruptSafeUnlock(&OS_task_table_mut, &previous);
    ...
    */
    ...
    OS_task_table[possible_taskid].events = 0;
    OS_task_table[possible_taskid].desired_events = 0;
    OS_task_table[possible_taskid].event_options = 0;
...
int32 OS_TaskDelete (uint32 task_id)
{
    ...
    */
    ...
    OS_task_table[task_id].events = 0;
    OS_task_table[task_id].desired_events = 0;
    OS_task_table[task_id].event_options = 0;
    pthread_mutex_destroy(&(OS_task_table[task_id].event_mutex));
    pthread_cond_destroy(&(OS_task_table[task_id].event_cond));
                       ...
                       uint32 data_size, uint32 flags)
{
   ...
   *queue_id = possible_qid;
    ...
    OS_queue_table[*queue_id].events = 0;
    OS_queue_table[*queue_id].event_taskid = OS_MAX_TASKS;
...
int32 OS_QueuePut (uint32 queue_id, void *data, uint32 size, uint32 flags)
{
   ...
   */
   ...
   if (OS_queue_table[queue_id].events != 0)
   {
       OS_EventSend(OS_queue_table[queue_id].event_taskid, OS_queue_table[queue_id].events);
   }
...
/*---------------------------------------------------------------------------------------
   Name: OS_QueueEventStart

   Purpose: This function makes the specified queue start sending the requested events
            to the calling task.

   Returns: OS_ERR_INVALID_ID if the id passed in is not a valid queue
            OS_EVENT_INVALID_ARGS if 0 is passed as event parameter
            OS_EVENT_ALREADY_REGISTERED if the semaphore is already sending events
            OS_ERRROR if the OS call failed
            OS_SUCCESS if success
---------------------------------------------------------------------------------------*/
int32 OS_QueueEventStart (uint32 queue_id, uint32 events)
{
    sigset_t   previous;
    sigset_t   mask;
    uint32     task_id;
    ...
    if (OS_queue_table[queue_id].event_taskid < OS_MAX_TASKS)
    {
        status = OS_EVENT_ALREADY_REGISTERED;
    }
    else
    {
        OS_queue_table[queue_id].event_taskid = task_id;
        OS_queue_table[queue_id].events = events;
    }
...
/*---------------------------------------------------------------------------------------
   Name: OS_QueueEventStop

   Purpose: This function stops the specified queue from sending events

   Returns: OS_ERR_INVALID_ID if the id passed in is not a valid queue
            OS_ERRROR if the OS call failed
            OS_SUCCESS if success
---------------------------------------------------------------------------------------*/
int32 OS_QueueEventStop (uint32 queue_id)
{
    sigset_t   previous;
    sigset_t   mask;
    uint32     task_id;
    ...
    if (task_id == OS_queue_table[queue_id].event_taskid)
    {
        OS_queue_table[queue_id].event_taskid = OS_MAX_TASKS;
        OS_queue_table[queue_id].events = 0;
    }
             ...
             */
             ...
             if ( Status == 0 )
             {
                ...
                *sem_id = possible_semid;
                ...
                OS_bin_sem_table[*sem_id].events = 0;
                OS_bin_sem_table[*sem_id].event_taskid = OS_MAX_TASKS;
...
int32 OS_BinSemGive ( uint32 sem_id )
{
    ...
    */
    ...
    if (OS_bin_sem_table[sem_id].events != 0)
    {
        OS_EventSend(OS_bin_sem_table[sem_id].event_taskid, OS_bin_sem_table[sem_id].events);
    }
...
/*---------------------------------------------------------------------------------------
    Name: OS_BinSemEventStart

    Purpose: This function makes the specified binary semaphore start sending events to
             the calling task.
    ...
    Returns: OS_ERR_INVALID_ID if the id passed in is not a valid semaphore
             ...
             OS_SUCCESS if success
---------------------------------------------------------------------------------------*/
int32 OS_BinSemEventStart (uint32 sem_id, uint32 events)
{
    sigset_t   previous;
    sigset_t   mask;
    uint32     task_id;
    ...
    if (OS_bin_sem_table[sem_id].event_taskid < OS_MAX_TASKS)
    {
        status = OS_EVENT_ALREADY_REGISTERED;
    }
    else
    {
        OS_bin_sem_table[sem_id].event_taskid = task_id;
        OS_bin_sem_table[sem_id].events = events;
    }
...
/*---------------------------------------------------------------------------------------
   Name: OS_BinSemEventStop

   Purpose: This function stops the specified binary semaphore from sending events

   Returns: OS_ERR_INVALID_ID if the id passed in is not a valid semaphore
            OS_ERRROR if the OS call failed
            OS_SUCCESS if success
---------------------------------------------------------------------------------------*/
int32 OS_BinSemEventStop (uint32 sem_id)
{
    sigset_t   previous;
    sigset_t   mask;
    uint32     task_id;
    ...
    if (task_id == OS_bin_sem_table[sem_id].event_taskid)
    {
        OS_bin_sem_table[sem_id].event_taskid = OS_MAX_TASKS;
        OS_bin_sem_table[sem_id].events = 0;
    }
             ...
             */
             ...
             if ( Status == 0 )
             {
                ...
                *sem_id = possible_semid;
                ...
                OS_count_sem_table[*sem_id].events = 0;
                OS_count_sem_table[*sem_id].event_taskid = OS_MAX_TASKS;
...
int32 OS_CountSemGive ( uint32 sem_id )
{
    ...
    */
    ...
    if (OS_count_sem_table[sem_id].events != 0)
    {
        OS_EventSend(OS_count_sem_table[sem_id].event_taskid, OS_count_sem_table[sem_id].events);
    }
...
/*---------------------------------------------------------------------------------------
    Name: OS_CountSemEventStart

    Purpose: This function makes the specified counting semaphore start sending events to
             the calling task.
    ...
    Returns: OS_ERR_INVALID_ID if the id passed in is not a valid semaphore
             ...
             OS_SUCCESS if success
---------------------------------------------------------------------------------------*/
int32 OS_CountSemEventStart (uint32 sem_id, uint32 events)
{
    sigset_t   previous;
    sigset_t   mask;
    uint32     task_id;
    ...
    if (OS_count_sem_table[sem_id].event_taskid < OS_MAX_TASKS)
    {
        status = OS_EVENT_ALREADY_REGISTERED;
    }
    else
    {
        OS_count_sem_table[sem_id].event_taskid = task_id;
        OS_count_sem_table[sem_id].events = events;
    }
...
/*---------------------------------------------------------------------------------------
   Name: OS_CountSemEventStop

   Purpose: This function stops the specified counting semaphore from sending events

   Returns: OS_ERR_INVALID_ID if the id passed in is not a valid semaphore
            OS_ERRROR if the OS call failed
            OS_SUCCESS if success
---------------------------------------------------------------------------------------*/
int32 OS_CountSemEventStop (uint32 sem_id)
{
    sigset_t   previous;
    sigset_t   mask;
    uint32     task_id;
    ...
    if (task_id == OS_count_sem_table[sem_id].event_taskid)
    {
        OS_count_sem_table[sem_id].event_taskid = OS_MAX_TASKS;
        OS_count_sem_table[sem_id].events = 0;
    }
...
/*---------------------------------------------------------------------------------------
   ...
   Purpose: This function sends the specified events to a task.
---------------------------------------------------------------------------------------*/

int32 OS_EventSend(uint32 task_id, uint32 events)
{
    int32     ret = 0;
    sigset_t  previous;
    sigset_t  mask;
    ...
    /* Get the condvar mutex */
    OS_InterruptSafeLock(&(OS_task_table[task_id].event_mutex), &mask, &previous);

    /* Set the desired event bits for the task */
    OS_task_table[task_id].events |= events;

    /* Signal the task if it's waiting for events and we've met its conditions */
    if (OS_task_table[task_id].desired_events != 0)
    {
        masked_events = OS_task_table[task_id].events & OS_task_table[task_id].desired_events;

        if ((OS_task_table[task_id].event_options == OS_EVENT_ALL &&
            masked_events == OS_task_table[task_id].desired_events) ||
            (OS_task_table[task_id].event_options == OS_EVENT_ANY &&
            masked_events != 0))
        {
            ret = pthread_cond_signal(&OS_task_table[task_id].event_cond);
        }
    }

    OS_InterruptSafeUnlock(&(OS_task_table[task_id].event_mutex), &previous);
...
/*---------------------------------------------------------------------------------------
   Name: OS_EventReceive

   Purpose: This function waits for the specified events. The calling thread will block
            until any or all of the requested events have occurred.
---------------------------------------------------------------------------------------*/

int32 OS_EventReceive(uint32 events, int8 options, int32 timeout, uint32 *events_received)
{
    sigset_t  previous;
    sigset_t  mask;
    uint32    task_id;
    ...
    if (options != OS_EVENT_ALL && options != OS_EVENT_ANY)
    {
        return OS_EVENT_INVALID_ARGS;
    }
    ...
    /* Lock the condvar mutex and populate event information */
    OS_InterruptSafeLock(&(OS_task_table[task_id].event_mutex), &mask, &previous);

    masked_events = OS_task_table[task_id].events & events;

    /* Check to see if the desired events are already available */
    if ((options == OS_EVENT_ALL && masked_events == events) ||
        (options == OS_EVENT_ANY && masked_events != 0))
    {
        /* Tell the user what they got */
        *events_received = masked_events;

        /* Switch off all of the received events */
        OS_task_table[task_id].events &= ~masked_events;
    ...
    /* We need to actually block, populate the wait parameters */
    OS_task_table[task_id].desired_events = events;
    OS_task_table[task_id].event_options = options;

    if (timeout == OS_PEND)
    {
        status = pthread_cond_wait(&OS_task_table[task_id].event_cond, &OS_task_table[task_id].event_mutex);
    }
    else
    {
        struct timespec ts;
        ...
        status = pthread_cond_timedwait(&OS_task_table[task_id].event_cond, &OS_task_table[task_id].event_mutex, &ts);
    }

    /* Now that we're done, clear out the event parameters */
    OS_task_table[task_id].desired_events = 0;
    OS_task_table[task_id].event_options = 0;

    /* Record and mask out received events */
    masked_events = OS_task_table[task_id].events & events;
    OS_task_table[task_id].events &= ~masked_events;
    ...
    *events_received = masked_events;
    ...
    if (status == ETIMEDOUT)
    {
        if (timeout == OS_CHECK)
        {
            return OS_EVENT_NOT_ALL_EVENTS;
        }
        else
        {
