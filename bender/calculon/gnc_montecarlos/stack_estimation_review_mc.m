% Lander6 Estimation Review Monte Carlo
...
for i_count=1:length(mc_i)
    ...
    if exist(strcat(name_mc,'.mat'))
        load( name_mc );
    else
        continue
    end
    mc_all_initial(i_count,:) = extract_mcall_initial_from_workspace( imc );
    mc_all_final(i_count,:) = extract_mcall_final_from_workspace( imc );
    t = tout; 
