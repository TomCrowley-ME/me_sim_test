                      ...
                      uint32 stack_size, uint32 priority, uint32 flags)
{
    ...
    {
#if defined(_ARM_) && defined(VFPVARIANT)
        LocalFlags = VX_VFP_TASK;
#else
...
int32 OS_QueueEventStart (uint32 queue_id, uint32 events)
{
    ...
    vx_status = msgQEvStart(OS_queue_table[queue_id].id, events, EVENTS_SEND_IF_FREE);

    if (vx_status != OK)
    {
        if (errno == S_eventLib_ALREADY_REGISTERED)
        {
...
int32 OS_QueueEventStop (uint32 queue_id)
{
    ...
    vx_status = msgQEvStop(OS_queue_table[queue_id].id);
...
int32 OS_BinSemEventStart (uint32 sem_id, uint32 events)
{
    ...
    vx_status = semEvStart(OS_bin_sem_table[sem_id].id, events, EVENTS_SEND_IF_FREE);
...
int32 OS_BinSemEventStop (uint32 sem_id)
{
    ...
    vx_status = semEvStop(OS_bin_sem_table[sem_id].id);
...
int32 OS_CountSemEventStart (uint32 sem_id, uint32 events)
{
    ...
    vx_status = semEvStart(OS_count_sem_table[sem_id].id, events, EVENTS_SEND_IF_FREE);
...
int32 OS_CountSemEventStop (uint32 sem_id)
{
    ...
    vx_status = semEvStop(OS_count_sem_table[sem_id].id);
...
int32 OS_EventSend(uint32 task_id, uint32 events)
{
    ...
    vx_status = eventSend(OS_task_table[task_id].id, events);
...
int32 OS_EventReceive(uint32 events, int8 options, int32 timeout, uint32 *events_received)
{
    ...
    {
        case OS_EVENT_ALL:
            vx_options = EVENTS_WAIT_ALL;
            break;
        case OS_EVENT_ANY:
            vx_options = EVENTS_WAIT_ANY;
            break;
        default:
            return OS_EVENT_INVALID_ARGS;
    }
    ...
    vx_status = eventReceive(events, vx_options, sys_ticks, (UINT32*)events_received);

    if (vx_status != OK)
    {
        if (errno == S_eventLib_TIMEOUT)
        {
            return OS_EVENT_TIMEOUT;
        }
        else if (errno == S_eventLib_NOT_ALL_EVENTS)
        {
