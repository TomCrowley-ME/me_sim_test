...
int32 GetTblDefInfo(void)
{
    ...
    else
    {
        ...
        /* If dealing with a shared object or executable, we need to adjust down by the base virtual address of the section */
        if (ElfHeader.e_type == ET_DYN || ElfHeader.e_type == ET_EXEC) {
            SeekOffset -= (uint32)SectionHeaderPtrs[SymbolPtrs[TblDefSymbolIndex]->st_shndx]->sh_addr;
        }
