...
load_model mtv1_with_slosh_with_valve_delay;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mc_n = 400;  % User set to total number of MC cases
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
lateral_cm_x_dry_max = 0.0003;
lateral_cm_y_dry_max = 0.0003;
lateral_cm_z_dry_max = 0.0003;
lateral_cm_x_dry_min =-0.0003;
lateral_cm_y_dry_min =-0.0003;
lateral_cm_z_dry_min = 0.0003;
...
wind_speed_min   = 0.0;
wind_speed_max   = 5.0;
sigma=(wind_speed_max-wind_speed_min)/3;
wind_speed       = abs(sigma*randn(mc_n,1));
...
gust_magnitude_max = 3.0;
sigma=(gust_magnitude_max-gust_magnitude_min)/3;
gust_magnitude     = abs(sigma*randn(mc_n,1));
gust_start_min     = 15.0;
gust_start_max     = 25.0;
gust_start         = gust_start_min + (gust_start_max - gust_start_min)*rand(mc_n,1);
gust_length_min    = 10.0;
gust_length_max    = 15.0;
gust_length        = gust_length_min + (gust_length_max - gust_length_min)*rand(mc_n,1);
gust_direction_min = -180.0;
gust_direction_max =  180.0;
...
fluid_damping_min = 0.0;
fluid_damping_max = 0.01;
sigma=(fluid_damping_max - fluid_damping_min)/3;
fluid_damping     =  abs(sigma*randn(mc_n,1));

% set peak pendulum length/tank radius ratio, nominally set to 100%
pendulum_length_min = 0.9;
pendulum_length_max = 1.1;
mid=(pendulum_length_max+pendulum_length_min)/2;sigma=(pendulum_length_max-pendulum_length_min)/3;
pendulum_length     =  mid+sigma*randn(mc_n,1);

% set peak pendulum mass fraction, nominally set to 100%
pendulum_mass_min   = 0.9;
pendulum_mass_max   = 1.1;
mid=(pendulum_mass_max+pendulum_mass_min)/2;sigma=(pendulum_mass_max-pendulum_mass_min)/3;
pendulum_mass       =  mid+sigma*randn(mc_n,1);

% set peak hinge height/tank radius ratio, nominally set to 100%
hinge_height_min = 0.9;
hinge_height_max = 1.1;
mid=(hinge_height_max+hinge_height_min)/2;sigma=(hinge_height_max-hinge_height_min)/3;
hinge_height     =  mid+sigma*randn(mc_n,1);

% set tank drain efficiencies, nominally set to 100%
drain_efficiency_min = 0.9;
drain_efficiency_max = 1.1;
mid=(drain_efficiency_max+drain_efficiency_min)/2;sigma=(drain_efficiency_max-drain_efficiency_min)/3;
drain_efficiency     = mid+sigma*randn(mc_n,4);

% set thruster delay, msec, nominally set to 0
acs_thruster_delay_min = 0.0;
acs_thruster_delay_max = 30.0;
sigma=(acs_thruster_delay_max-acs_thruster_delay_min)/3;
acs_thruster_delay     = abs(sigma*randn(mc_n,12)/1000);

% set thruster fire time bias, msec, nominally set to 0
acs_thruster_bias_min = 0.0;
acs_thruster_bias_max = 20.0;
mid=(acs_thruster_bias_max+acs_thruster_bias_min)/2;sigma=(acs_thruster_bias_max-acs_thruster_bias_min)/3;
acs_thruster_bias     = (mid + sigma*randn(mc_n,12))/1000;

% set random misalignment angles and azimuth angles for 25 thrusters
misalignment_ang_deg_1sigma = 0.125;
...
% set throttle to thrust gain
throttle_to_thrust_min = 0.98;
throttle_to_thrust_max = 1.02;
% throttle_to_thrust     = throttle_to_thrust_min + (throttle_to_thrust_max - throttle_to_thrust_min)*rand(mc_n,1);
mid=(throttle_to_thrust_max+throttle_to_thrust_min)/2;sigma=(throttle_to_thrust_max-throttle_to_thrust_min)/3;
throttle_to_thrust     =  mid+sigma*randn(mc_n,1);

% set throttle to mdot gain
throttle_to_mdot_min = 0.98;
throttle_to_mdot_max = 1.02;
% throttle_to_mdot     = throttle_to_mdot_min + (throttle_to_mdot_max - throttle_to_mdot_min)*rand(mc_n,1);
mid=(throttle_to_mdot_max+throttle_to_mdot_min)/2;sigma=(throttle_to_mdot_max-throttle_to_mdot_min)/3;
throttle_to_mdot     =  mid+sigma*randn(mc_n,1);

% set mass estimate bias
mass_estimate_bias_min = -1.0;
mass_estimate_bias_max =  1.0;
...
ivars = struct( 'ldr_2k_percent_error',1,...
                ...
                'acs_thruster_delay',[85:96],...
                'acs_thruster_bias',[97:108],...
                'throttle_to_thrust',109,'throttle_to_mdot',110,...
                'mass_estimate_bias',111,...
                'n',111);
...
for i2mc = 1 : length(mc_i)
    ...
    % EGC Vertical Control
    egc_vel_error_gain = 10*[0.0338    0.2401    0.4521    0.2401    0.0338];
...
% set mIRU IMU seeds
    ...
    % %%%%%%%%%%%%%
        ...
        ppc_coarse_thr_delay = uint8(zeros(12,1));
        ppc_fine_thr_delay   = uint8(zeros(12,1));
        ppc_coarse_thr_bias  = uint8(zeros(12,1));

        % uncomment this section to set acs thruster delay,
        if ~isnan( mc_6dof_variables(imc,ivars.acs_thruster_delay) )
            ppc_coarse_thr_delay = uint8(mc_6dof_variables(imc,ivars.acs_thruster_delay));
        end

        % uncomment this section to set acs thruster fire time bias,
        if ~isnan( mc_6dof_variables(imc,ivars.acs_thruster_bias) )
            ppc_coarse_thr_bias = uint8(mc_6dof_variables(imc,ivars.acs_thruster_bias));
        end
        
        % EGC Vertical Control
        if ~isnan( mc_6dof_variables(imc,ivars.throttle_to_thrust) )
            tpl_egc_throttle_to_thrust_gain = mc_6dof_variables(imc,ivars.throttle_to_thrust);
        end
        if ~isnan( mc_6dof_variables(imc,ivars.throttle_to_mdot) )
            tpl_egc_throttle_to_mdot_gain   = mc_6dof_variables(imc,ivars.throttle_to_mdot);
        end
    ...
    sim('mtv1_with_slosh_with_valve_delay');
 %%
...
%    mc_traj_data.gdn_t      = telem.gdn.seq_t;
    ...
    mc_traj_data.htp_used   = truth.sim.prop_used_main;
    mc_traj_data.gn2_used   = truth.sim.prop_used_acs;
    mc_traj_data.egc_throttle = telem.egc.throttle;
